...
# FIXED: UUID to string conversion for LlamaIndex compatibility
...
class PersonNameDetector:
    ...
    def __init__(self):
        ...
        self.person_patterns = [
            ...
            # Names with prefixes: Van der, De, Di, Du, Da, Del, Della, Von, Zu
...
class LlamaIndexRetriever(BaseRetriever):
    ...
    def _ensure_node_id_is_string(self, node) -> None:
        """Ensure node.node.id_ is string (convert UUID if needed)"""
        try:
            if hasattr(node, 'node'):
                if hasattr(node.node, 'id_') and isinstance(node.node.id_, uuid.UUID):
                    node.node.id_ = str(node.node.id_)

                # Also ensure node_id is string if it exists
                if hasattr(node.node, 'node_id') and isinstance(node.node.node_id, uuid.UUID):
                    node.node.node_id = str(node.node.node_id)

                # Clean metadata UUIDs
                if hasattr(node.node, 'metadata') and isinstance(node.node.metadata, dict):
                    node.node.metadata = self._convert_uuid_to_string(node.node.metadata)
        except Exception as e:
            logger.warning(f"Error converting UUID to string in node: {e}")
    ...
    async def retrieve(self, query: str, top_k: int = 10, similarity_threshold: float = None, **kwargs) -> List[RetrievalResult]:
        ...
        try:
            ...
            # 🔧 CRITICAL FIX: Use raw vector store query instead of LlamaIndex retriever
            # This bypasses TextNode creation and UUID validation
            try:
                ...
                # Get index's vector store
                vector_store = self.index._vector_store
                query_result = vector_store.query(vector_query)

                logger.info(f"   Vector: {len(query_result.nodes)} candidates retrieved (raw)")

                # Convert raw results to our RetrievalResult format
                results = []
                for i, node in enumerate(query_result.nodes):
                    try:
                        ...
                        # Get metadata (convert UUIDs)
                        metadata = {}
                        if hasattr(actual_node, 'metadata'):
                            metadata = self._convert_uuid_to_string(actual_node.metadata)

                        # Get content
                        content = ''
                        if hasattr(actual_node, 'text'):
                            content = actual_node.text
                        elif hasattr(actual_node, 'get_content'):
                            content = actual_node.get_content()
                        else:
                            content = str(actual_node)

                        # Get score
                        similarity_score = 0.0
                        if hasattr(node, 'score'):
                            similarity_score = node.score
                        elif query_result.similarities and i < len(query_result.similarities):
                            similarity_score = query_result.similarities[i]
                        ...
                        # Content validation
                        if not self._is_content_relevant(query, content, extracted_entity):
                            continue
                        ...
                        logger.warning(f"Error processing raw node {i}: {e}")
                        continue
                ...
                logger.error(f"❌ Raw vector search failed: {e}")
                # Fallback to standard retriever if raw query fails
                raise
...
class DatabaseRetriever(BaseRetriever):
    ...
    async def _exact_phrase_search(self, cur, query: str, limit: int) -> List[RetrievalResult]:
        ...
        for row in rows:
            try:
                ...
                # 🔧 FIX: Ensure document_id is string
                doc_id = row.get('id', '')
    ...
    async def _person_name_search(self, cur, query: str, limit: int, exclude_ids: List[str] = None) -> List[RetrievalResult]:
        ...
        exclude_condition = ""
        if exclude_ids:
            # 🔧 FIX: Ensure exclude_ids are strings
...
class MultiStrategyRetriever:
    ...
    async def _retrieve_with_vector_variants(
            self,
            queries: List[str],
            top_k: int,
