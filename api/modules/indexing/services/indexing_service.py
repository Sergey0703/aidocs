...
class IndexingService:
    ...
    async def _run_conversion_stage(self, task: IndexingTaskState, documents_dir: Optional[str]):
        """Run document conversion stage (Part 1)"""
        ...
        try:
            ...
            for i, file_path in enumerate(files_to_process, 1):
                ...
                if success:
                    ...
                    task.statistics.documents_converted += 1
                else:
                    task.failed_files += 1
                    task.errors.append(f"Conversion failed: {file_path} - {error}")
                
                # Update progress
                task.update_progress((i / task.total_files) * 100)
                task.update_speed()
            
            logger.info(f"✅ Conversion stage completed: {task.processed_files}/{task.total_files} files")
    ...
    async def _run_indexing_stage(
        ...
        try:
            ...
            logger.info(f"   Loaded {task.total_files} documents")
            ...
            valid_nodes, invalid_nodes, node_stats = create_and_filter_chunks_enhanced(
                documents, config, node_parser, progress_tracker
            )
            
            task.total_chunks = node_stats['total_nodes_created']
            task.statistics.chunks_created = node_stats['total_nodes_created']
            task.statistics.chunks_valid = node_stats['valid_nodes']
            task.statistics.chunks_invalid = node_stats['invalid_nodes']
            
            logger.info(f"   Created {task.total_chunks} chunks ({task.statistics.chunks_valid} valid)")
            
            if task.statistics.chunks_valid == 0:
                logger.warning("   No valid chunks to index")
                return
            
            # Sub-stage: Embedding and Saving
            ...
            embedding_processor = create_embedding_processor(
                embed_model, vector_store, config
            )
            ...
            if delete_existing:
                ...
                deletion_info = db_manager.safe_deletion_dialog(files_to_process, incremental_mode=False)
                logger.info(f"   Deleted {deletion_info.get('records_deleted', 0)} existing records")
            
            # Process batches with progress tracking
            batch_settings = config.get_batch_settings()
            total_nodes = len(valid_nodes)
            
            task.total_batches = (total_nodes + config.PROCESSING_BATCH_SIZE - 1) // config.PROCESSING_BATCH_SIZE
            
            # Custom batch processing with progress updates
            for i in range(0, total_nodes, config.PROCESSING_BATCH_SIZE):
                if task.cancelled:
                    break
                
                batch_nodes = valid_nodes[i:i + config.PROCESSING_BATCH_SIZE]
                batch_num = i // config.PROCESSING_BATCH_SIZE + 1
                
                task.current_batch = batch_num
                task.current_file = f"Batch {batch_num}/{task.total_batches}"
                
                # Process batch
                batch_result = batch_processor.process_batch(
                    batch_nodes, batch_num, task.total_batches,
                    batch_settings['embedding_batch_size'],
                    batch_settings['db_batch_size']
                )
                
                # Update statistics
                task.processed_chunks += batch_result['records_saved']
                task.statistics.chunks_saved += batch_result['records_saved']
                task.statistics.gemini_api_calls += batch_result.get('gemini_api_calls', 0)
                
                # Update progress
                progress = (i + len(batch_nodes)) / total_nodes * 100
